...
Service/types/1.0"><Message><PostNotificationsAckRequest xmlns="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0">
<notificationList><inn>423050468026</inn><messageId>2</messageId></notificationList></PostNotificationsAckRequest></Message>
...
def r = utils.xml(req).Body.SendMessageRequest.Message.PostNotificationsAckRequest.notificationList
def inn = r.inn.text()
...
if (user.params.reg == 0) {
    ...
    messageId.each { mId ->
        user.params.notifs.find { it.id.toString() == mId.text() }.status = 1
    }
    params.put("Message", response())
}

def response() {
    return """<PostNotificationsAckResponse xmlns="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0" xsi:schemaLocation="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0 schema.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <status>OK</status>
</PostNotificationsAckResponse>"""
}
//---END RESPONSE SCRIPT---//
