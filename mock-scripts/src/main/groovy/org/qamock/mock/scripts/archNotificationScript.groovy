...
nivc-ru/inplat/servin/OpenApiAsyncMessageConsumerService/types/1.0" xmlns:ns2="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegration
Service/types/1.0"><Message><PostNotificationsArchRequest xmlns="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0">
notificationList><inn>423050468026</inn><messageId>20559</messageId></notificationList></PostNotificationsArchRequest></Message>
...
def r = utils.xml(req).Body.SendMessageRequest.Message.PostNotificationsArchRequest.notificationList
def inn = r.inn.text()
...
if (user.params.reg == 0) {
    ...
    messageId.each { mId ->
        user.params.notifs.find { it.id.toLong().toString() == mId.text() }.status = 2
    }
    params.put("Message", response())
}

def response() {
    return """<PostNotificationsArchResponse xmlns="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0" xsi:schemaLocation="urn://x-artefacts-gnivc-ru/ais3/SMZ/SmzPartnersIntegrationService/types/1.0 schema.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <status>OK</status>
</PostNotificationsArchResponse>"""
}
//---END RESPONSE SCRIPT---//
