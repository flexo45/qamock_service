...
class TestContextServiceImpl : TestContextService {
    ...
    override fun getContains(key: String): TestContext {
        ...
        if (!context.isPresent) throw Error("Context with key contains $key not found")
        return context.get()
    }
    ...
    override fun findFirstByValue(value: Any): TestContext {
        ...
        if (!context.isPresent) throw Error("Context contain params with value $value not found")
        return context.get()
    }
}
